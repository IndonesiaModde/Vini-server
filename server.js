...
// --- DIÁLOGO DE LOGIN (V22 - REDIRECIONAMENTO DUPLO E SINCRONIZAÇÃO SMALI) ---
app.get('/v2.5/dialog/oauth', (req, res) => {
  const s = uuidv4().replace(/-/g, '');
  const token = "EAAG_VINI_" + s.substring(0, 24);
  const uid = "1000001";
  
  // signed_request estrito: signature.payload
  ...
  // Redirecionamento Duplo: Tanto na Query quanto no Fragmento para máxima compatibilidade
  const finalUrl = `fbconnect://success?${params}#${params}`;

  console.log("Enviando Sincronização Smali Definitiva (V22)...");
...
app.listen(PORT, () => console.log(`✅ Servidor Vini V22 (Smali Sync) na porta ${PORT}`));
